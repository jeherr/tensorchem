...
def read_multi_sp_data(filename):
    n_atoms = None
    mset = None
    atomic_nums = []
    coords = []
            ...
            while True:
                ...
                # charges
                    ...
                    if len(atomic_nums) == len(energies) == len(forces) == len(coords) == len(dipoles) == len(
                        ...
                        meta_trajectory.append(mset.build_geom(coords[-1], mol_labels, atom_labels))
...
for mol in glob.glob("/home/jeherr/tensorchem/data/chemspider_data/outputs/meta/crc/*.out"):
    head, name = os.path.split(mol)
    mol_mset = read_multi_sp_data(mol)
    if mol_mset is not None:
        mol_mset.save(filename=os.path.join("/mnt/sdb1/jeherr/chemspider_data/expanded_msets/meta/crc/",
                                        ".".join((name[:-4], "mset"))))
